...
const useChatSocket = ({
  ...
  useEffect(() => {
    ...
    const handleE2EEUpdated = ({ chatId, e2eeEnabled, chat }) => {
      queryClient.setQueryData(["chats"], (prev) => {
        if (!prev?.chats) return prev;
        return {
          ...prev,
          chats: prev.chats.map((c) =>
            c._id === chatId ? { ...c, e2eeEnabled, ...chat } : c
          ),
        };
      });
      ...
      if (selectedChat?._id === chatId) {
        setSelectedChat((prev) =>
          prev ? { ...prev, e2eeEnabled, ...chat } : prev
        );
      }
    };
    ...
    socket.on("e2ee-updated", handleE2EEUpdated);
    ...
    return () => {
      ...
      socket.off("e2ee-updated", handleE2EEUpdated);
