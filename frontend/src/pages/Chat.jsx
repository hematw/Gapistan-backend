...
import { shouldUseE2EE } from "../utils/e2ee";
...
function Chat() {
  ...
  const sendMessage = useCallback(
    async (formdata) => {
      ...
      let iv = null;
      const useE2EE = shouldUseE2EE(selectedChat);

      if (!isTextEmpty && useE2EE) {
        if (selectedChat?.isGroup) {
          ...
          if (groupChatKey) {
            ...
            return addToast({
              ...
              description: "Group encryption key not ready. Try again shortly.",
              color: "danger",
            });
          }
        } else if (privateKey && selectedChat) {
          let otherUser;
          try {
  ...
  const handleSocketResponse = async ({ message, error, data }) => {
    ...
    if (!error) {
      ...
      if (data.text) {
        if (useE2EE && data.iv && privateKey && selectedChat) {
          try {
            let aesKey;
            ...
            const msgIv = new Uint8Array(data.iv);
            decryptedOutgoingText = await decryptMessage(aesKey, data.text, msgIv);
          } catch (err) {
            console.error("Error decrypting sent message:", err);
            decryptedOutgoingText = "[Decryption Failed for Sent Message]";
          }
        } else {
          decryptedOutgoingText = data.text;
        }
      }
  ...
  // E2EE: Decrypt incoming messages (or use plaintext when E2EE is off)
  useEffect(() => {
    async function decryptAndSetTimeline() {
      if (!chatTimeline?.messages || !selectedChat) return;

      const useE2EE = shouldUseE2EE(selectedChat);
      if (useE2EE && !privateKey) return;
      if (useE2EE && selectedChat.isGroup && !groupChatKeys[selectedChat._id]) return;
        ...
        chatTimeline.messages.map(async (group) => {
          const newItems = await Promise.all(
            group.items.map(async (msg) => {
              if (!msg.text || msg.decryptedText) return msg;
              ...
              try {
                ...
                if (msg.replyTo?.text) {
                  if (msg.replyTo.iv) {
                    ...
                    decryptedReplyToText = msg.replyTo.text;
                  }
                }
                  ...
                  <ChatHeader
                    ...
                    onE2EEChange={(enabled) => {
                      setSelectedChat((prev) =>
                        prev ? { ...prev, e2eeEnabled: enabled } : prev
                      );
                    }}
                  />
                  <ChatTimeline
